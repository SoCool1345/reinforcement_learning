...
from sc2.main import run_game, run_multiple_games, GameMatch,_play_game_ai
...
transaction_file = f'transaction{args.rank}.pkl'
class WorkerRushBot(BotAI):
    ...
    async def on_step(self, iteration: int):
        ...
        if transaction is not None and transaction['action'] is not None:
            ...
            try:
                if action == 0:
                        ...
                        if not have_builded:
                            ...
                            for vespene in self.vespene_geyser.closer_than(15, nexus):
                                if self.can_afford(UnitTypeId.ASSIMILATOR) and not self.structures(
                                        UnitTypeId.ASSIMILATOR).closer_than(2, vespene).exists:
                    ...
                    for nexus in self.townhalls:
                        ...
                        if not self.structures(UnitTypeId.GATEWAY) and self.already_pending(UnitTypeId.GATEWAY) == 0:
                            if self.can_afford(UnitTypeId.GATEWAY):
                                ...
                                # print('建造传送门')
                        # 控制核心
                        if not self.structures(UnitTypeId.CYBERNETICSCORE) and self.already_pending(
                    ...
                    if self.can_afford(UnitTypeId.STARGATE):
                        await self.build(UnitTypeId.STARGATE, near=self.structures(UnitTypeId.PYLON).closest_to(
                            np.random.choice(self.townhalls)))
                        # print('建造星门')
            ...
            map = cv2.copyMakeBorder(map, 224 - map.shape[0], 0, 224 - map.shape[1], 0, cv2.BORDER_CONSTANT,
                                     value=(0, 0, 0))


            transaction = {'observation': map, 'reward': reward, 'action': None, 'done': False}

            with open(transaction_file, 'wb') as f:
